#!/usr/bin/env python3
"""
SPIKE COPY for testing the discussion-order design
(docs/superpowers/specs/discussion-order-design.md) against a real roster.

This is scripts/build_schedule.py with exactly the edits described in that
spec applied:
  1. New order_for_discussion() helper (uses the existing group_by_manager).
  2. build_schedule()'s pending[level] construction calls it instead of a
     bare group_by_manager loop.
  3. render_agenda_for_level()'s roster building walks the already-ordered
     list with run-length manager headers, instead of re-grouping.
  4. main() adds a "sensitive_review" key to the JSON summary for
     manager_proposed_rating 1/2, and does NOT write those into the .md files.

Everything else below is unchanged from the real script.
"""
import json
import argparse
import os
...
def build_schedule(data):
    ...
    # Pending queue per level, ordered for discussion (promo / rating-4 /
    # sensitive / remaining-by-manager - see order_for_discussion). Each level
    # is scheduled as ONE atomic block - it either runs in full on a given day
    # (everyone who's in it, together) or it doesn't run that day at all.
    pending = {}
